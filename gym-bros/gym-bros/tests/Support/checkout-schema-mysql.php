...
if (!preg_match('/^gym_bros_checkout_qa_[a-f0-9]{12}$/',$database)) { throw new RuntimeException('Nombre QA no valido.'); }
$conexion = config('database.connections.'.$original);
$conexion['database']=$database; $conexion['url']=null;
...
try {
    ...
    if (DB::connection()->getDatabaseName() !== $database) { throw new RuntimeException('Conexion QA incorrecta.'); }
    ...
    $migration->up();
    if (!Schema::hasTable('ordenes_compra') || !Schema::hasColumn('usuarios','correo_normalizado')) { throw new RuntimeException('Falta esquema.'); }
    $migration->down();
    if (Schema::hasTable('ordenes_compra')) { throw new RuntimeException('Rollback incompleto.'); }
    $migration->up();
