...
const plan = ref(null)
/*
 * La comida sale de la ficha del plan, que ya la trae: no hay endpoint propio
 * para una comida suelta y no se inventa uno.
 */
const comida = ref(null)
const enviando = ref(false)
const erroresServidor = ref({})
const mensajeError = ref('')
let solicitudActual = 0

/* Recurso auxiliar: si falla, el selector muestra su error y ofrece reintentar. */
const catalogo = ref([])
...
async function cargarPlan() {
  ...
  try {
    ...
    const encontrada = respuesta.comidas.find(
      (actual) => String(actual.id) === String(route.params.idComida),
    )
    if (!encontrada) {
      plan.value = respuesta
      mensajeError.value = 'La comida solicitada ya no forma parte de este plan.'
      estado.value = 'not-found'
      return
    }

    plan.value = respuesta
    comida.value = encontrada
...
async function guardar(datos) {
  ...
  try {
    await actualizarComida(route.params.idPlan, route.params.idComida, datos)
    await volverAlPlan({ notice: 'updated' })
  } catch (error) {
    if (error?.status === 422) {
      /*
      ...
      comida.value = null
      mensajeError.value = error?.message || 'La comida solicitada ya no existe.'
      estado.value = 'not-found'
    } else mensajeError.value = error?.message || 'No pudimos actualizar la comida.'
  } finally {
...
watch(
  [() => route.params.idPlan, () => route.params.idComida],
  () => {
    cargarPlan()
    cargarCatalogo()
  },
...
<template>
  <section class="comida-editor">
    <PageHeader
      ...
      :descripcion="
        comida
          ? `Actualiza la comida de las ${comida.horaSugerida || 'sin hora'} del plan.`
          : 'Actualiza una comida del horario del plan.'
      "
      seccion="Alimentación"
      :ruta-seccion="{ name: 'alimentacion-listado' }"
      etiqueta="Nutrición y planes"
      :migas="[
        plan?.usuario?.nombre ?? 'Plan',
        comida ? etiquetaDe(TIPOS_COMIDA, comida.tipoComida) : 'Comida',
        'Editar',
      ]"
    />
    ...
    <section v-if="estado === 'loading'" class="comida-editor__estado gb-tarjeta" aria-busy="true">
      ...
      <p>Cargando la comida…</p>
    </section>

    <ComidaForm
      v-else-if="estado === 'success' && comida"
      modo="edit"
      :valores-iniciales="comida"
      :enviando="enviando"
    ...
    >
      ...
      <div>
        ...
        <RouterLink
          class="btn btn-ghost"
          :to="{ name: 'plan-alimentacion-detalle', params: { id: route.params.idPlan } }"
        >
          Volver al plan
        </RouterLink>
      </div>
